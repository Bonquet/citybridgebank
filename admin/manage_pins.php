...
$pinStmt = $db->prepare('SELECT pin_type, is_active FROM user_pins WHERE user_id = ?');
$pinStmt->execute([$userId]);
$rows = $pinStmt->fetchAll();
$pins = [
    'authorization' => ['set' => false, 'active' => false],
    'payment' => ['set' => false, 'active' => false],
    'secure_pass' => ['set' => false, 'active' => false]
];
foreach ($rows as $r) {
...
<p style="color:var(--text-secondary)">Authentication, Payment, and Secure PINs are admin-managed. PIN values stay masked.</p>
